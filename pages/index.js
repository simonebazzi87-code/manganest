...
export default function MangaNest() {
  ...
  const addVolume = (sIndex) => {
    const volNum = prompt(
      "Numero volume singolo o intervallo (es. 1 o 3-7 o 0 o 0.5)"
    );
    if (!volNum) return;
  ...
  const lastVolumeNumber = (s) =>
    s.volumes.length ? Math.max(...s.volumes.map((v) => v.number)) : 0;
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto space-y-4 font-sans">
      ...
      {series.map((s, sIndex) => (
        <div
          key={sIndex}
          className="border rounded p-3 space-y-2 shadow-md bg-white"
        >
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold">
              {s.name} {s.favoriteBy.includes(currentReader) ? "❤️" : ""}
            </h2>
            <div className="flex gap-2">
              <button
                className="bg-gray-200 px-2 rounded"
                onClick={() => markAllRead(sIndex)}
              >
                Segna tutta letta
              </button>
              <button
              ...
              <button
                className="bg-gray-200 px-2 rounded"
                onClick={() => toggleFavoriteSeries(sIndex)}
              >
                ❤️ Serie
              </button>
            </div>
            ...
            >
              ...
              <div className="flex gap-4 items-center">
                <label className="flex items-center gap-1">
                  <input
                    type="checkbox"
                    checked={v.read[currentReader] || false}
                    onChange={() => toggleRead(sIndex, vIndex)}
                  />{" "}
                  {currentReader}
                </label>
                <button
                  className="bg-red-200 px-2 rounded"
                  onClick={() => toggleLastVolume(sIndex, vIndex)}
                >
                  Ultimo
                </button>
              </div>
      ...
      <div className="border rounded p-3 shadow-md bg-white">
        ...
        {series.map((s) => {
          const missing = s.volumes.filter(
            (v) =>
              !v.read[readers[0].name] || !v.read[readers[1].name]
          );
          if (missing.length === 0) return null;
          return (
            <div key={s.name} className="mb-2">
              <strong>{s.name}</strong>:{" "}
              {missing.map((v) => v.number).join(", ")} (ultimo volume
              registrato: {lastVolumeNumber(s)})
            </div>
          );
        })}
      </div>
            ...
            const csv = series
              .map((s) =>
                s.volumes
                  .map(
                    (v) =>
                      `${s.name},${v.number},${Object.keys(v.read)
                        .filter((r) => v.read[r])
                        .join("|")},${s.status},${s.authors.join(
                        "|"
                      )},${s.publisher},${v.isLast}`
                  )
                  .join("\n")
              )
              .join("\n");
